...
def main():
    ...
    with st.sidebar:
        ...
        use_ai = st.checkbox("🤖 AI分析を使用", value=bool(api_key), disabled=not bool(api_key))
        
        if not api_key:
            st.warning("⚠️ API キーを入力するとAI分析が有効になります")
        else:
    ...
    if tab == "🏠 ホーム":
        ...
        if not model:
            st.warning("💡 **ヒント:** サイドバーでGemini API キーを入力すると、より高度なAI分析が利用できます！")
